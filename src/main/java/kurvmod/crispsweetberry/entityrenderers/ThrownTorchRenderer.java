package kurvmod.crispsweetberry.entityrenderers;
...
/**
 * A <b>custom renderer for ThrownTorch</b>, which allows to <b>break the limitation of ThrownItemRenderer</b>.<br>
 * Still work in progress.
 */
public class ThrownTorchRenderer extends EntityRenderer<ThrownTorch>
{
    //Constants
    ...
    private static final float TORCH_SCALE = 0.75F;
    ...
    @Override
    public void render(@NotNull ThrownTorch entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight)
    {
        poseStack.pushPose();
     ...
     */
    @Override
    public @NotNull ResourceLocation getTextureLocation(@NotNull ThrownTorch entity)
    {
        if(entity.getTier() == ThrownTorch.TIER_GONE)//Only when ThrownTorch has contracted with water, then the method shall return a different sprite.
            return ResourceLocation.fromNamespaceAndPath(MOD_ID, TEXTURE_PATH + TEXTURE_DARK_STATE + TEXTURE_SUFFIX);
        return ResourceLocation.fromNamespaceAndPath(MOD_ID, TEXTURE_PATH +  TEXTURE_SUFFIX);
    }
}