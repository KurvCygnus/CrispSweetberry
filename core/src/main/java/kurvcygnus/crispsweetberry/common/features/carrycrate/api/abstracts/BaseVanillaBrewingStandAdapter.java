...
public abstract class BaseVanillaBrewingStandAdapter<E extends BrewingStandBlockEntity> extends AbstractBlockEntityCarryAdapter<E>
{
    ...
    @Override public void carryTick(@NotNull ServerLevel level, long carryingTime)
    {
        ...
        if(accessor.getFuel() == 0)
        {
            ...
            if(!fuelItem.is(Items.BLAZE_POWDER))
                return;
            
            accessor.setFuel(MAX_FUEL);
            fuelItem.shrink(1);
        ...
        final boolean canFinishBrew = carryingTime >= accessor.getBrewTime();
        ...
        {
            accessor.setBrewTime(MAX_BREWING_TIME);
            doBrew(level, this.blockEntity.getBlockPos());
        }
    }
